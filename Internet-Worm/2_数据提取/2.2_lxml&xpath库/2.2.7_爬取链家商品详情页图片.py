...
for i in range(1,2):
    ...
    listcontents = tree.xpath("//div//div//ul[@class='sellListContent']/li")
    for listcontent in listcontents:
        try:
            title = listcontent.xpath(".//div[@class='title']/a/text()")[0]
            link = listcontent.xpath(".//div[@class='title']/a/@href")[0]
            r_each = requests.get(link, headers=headers, timeout=30)
            # print(link)
            r_each.raise_for_status()
            r_each.encoding = r_each.apparent_encoding
            print(r.status_code)
            html_each = etree.HTML(r_each.text)
            plan_each = html_each.xpath("//div//ul[@class='smallpic']//li[@data-desc='户型图']/@data-pic")[0]
            print(plan_each)
            plan_data = requests.get(plan_each).content
            print(plan_data)
            name = title + ".jpg"
